...
from models.net import GatedCrossAttentionFusion
from envs.crypto_env import CryptoFusionEnv

LOG_DIR = "./logs_GatedCrossAttentionFusion/"
...
if __name__ == "__main__":
    ...
    # 5. 配置模型 (适配双通道输出的宽广特征)
    policy_kwargs = dict(
        features_extractor_class=GatedCrossAttentionFusion,
        features_extractor_kwargs=dict(
            sent_dim=3, social_dim=4, market_dim=5, hidden_dim=64, dropout=0.2
        ),
        # 既然我们输出了庞大丰富的特征向量(193维)，下游的 Actor(pi) 和 Critic(vf) 也需要加深
    ...
    print(" 训练结束！最优模型已保存至 logs_GatedCrossAttentionFusion/best_model.zip")